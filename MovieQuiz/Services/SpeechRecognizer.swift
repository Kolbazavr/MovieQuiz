...
actor SpeechRecognizer {
    ...
    private weak var delegate: VoiceCommandRecognizerDelegate?
    ...
    init(delegate: VoiceCommandRecognizerDelegate) {
    ...
    @MainActor func checkResult() {
        Task {
            if transcript.lowercased().contains("да") {
                print("Вроде я услышал >>>ДА<<<")
                self.resetTranscript()
                self.sendCommand(true)
                
                
            } else if transcript.lowercased().contains("нет") {
                print("Вроде я услышал >>>НЕТ<<<")
                self.resetTranscript()
